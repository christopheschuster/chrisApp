...
function getActualDataFrom(transactionBreakdownRows) {
  return transactionBreakdownRows.map((transactionBreakdownRow) => {
    const title = within(transactionBreakdownRow).getByTestId('transaction-breakdown-row-title');
    const value = within(transactionBreakdownRow).getByTestId('transaction-breakdown-row-value');
...
describe('TransactionBreakdown', () => {
  const store = configureMockStore()({
    chrisapp: {
      ...
      internalAccounts: mockState.chrisapp.internalAccounts,
  ...
  describe('with a typical non-EIP-1559 transaction', () => {
    it('renders properly', () => {
      ...
      expect(getActualDataFrom(getAllByTestId('transaction-breakdown-row'))).toStrictEqual([
        ['Nonce', '29'],
        ['Amount', '-0.01 ETH'],
        ...
        <TransactionBreakdown
          ...
          transaction={{
            txParams: {
              ...
              maxPriorityFeePerGas: '0x930c19d4',
              value: '0x2386f26fc10000',
            },
            txReceipt: {
              gasUsed: '7a1c',
              effectiveGasPrice: '0x930c19db',
            },
            baseFeePerGas: '0x7',
          }}
          primaryCurrency="-0.01 ETH"
  ...
  describe('with swap+send transaction', () => {
    ...
    it('renders properly', () => {
      const { getAllByTestId } = renderWithProvider(<TransactionBreakdown {...props} />, store);
